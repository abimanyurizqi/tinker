'use client'
import { useEffect, useMemo, useRef, useState, type CSSProperties, type Dispatch, type ReactNode, type SetStateAction, type UIEvent } from 'react'
import dynamic from 'next/dynamic'
import { AlertTriangle, ArrowLeftRight, Binary, Boxes, Braces, Check, ChevronDown, ChevronRight, ChevronUp, Clock, Code2, Coffee, Copy, Database, Dices, Download, FileCode2, FileDiff, FileJson, FileText, Fingerprint, Hash, KeyRound, Link, List, Moon, Minimize2, RotateCcw, Scissors, ShieldCheck, Sun, Table, Terminal, Wand2 } from 'lucide-react'
...
function prettyJson(x: string): string {
  try { return JSON.stringify(JSON.parse(x), null, 2) } catch { return x }
}

function prettyXml(x: string): string {
  const tags = x.replace(/<!--[\s\S]*?-->/g, m => m.replace(/\n/g, ' ')).match(/<[^>]+>|[^<]+/g) || []
  const lines: string[] = []
  let depth = 0
  ...
  for (const token of tags) {
    const t = token.trim()
    if (!t) continue
    if (t.startsWith('<![CDATA[') || t.startsWith('<?')) { lines.push(indent() + t); continue }
    if (t.startsWith('</')) { depth = Math.max(0, depth - 1); lines.push(indent() + t); continue }
    if (t.startsWith('<!--') || /\/>$/.test(t)) { lines.push(indent() + t); continue }
    if (t.startsWith('<')) { lines.push(indent() + t); depth += 1; continue }
    const text = t.split(/\s+/).join(' ').trim()
...
function OutPanel({ icon, title, label, foot, actions, children, style, className }: { icon: ReactNode; title: string; label?: string; foot?: ReactNode; actions?: ReactNode; children: ReactNode; style?: CSSProperties; className?: string }) {
  return <section className={`panel ${className ?? ''}`} style={style}><div className="panel-head"><div className="panel-title">{icon}<h2>{title}</h2>{label && <span className="format-label">{label}</span>}</div>{actions ? <div className="panel-actions">{actions}</div> : null}</div><div className="result-body">{children}</div>{foot ? <div className="panel-foot">{foot}</div> : null}</section>
}
...
export default function Home() {
  ...
  const diffNav = useRef<{ next: () => void; prev: () => void } | null>(null)
  ...
  const jdFmt = useMemo(() => [prettyJson(jdA), prettyJson(jdB)] as const, [jdA, jdB])
  const xdFmt = useMemo(() => [prettyXml(xdA), prettyXml(xdB)] as const, [xdA, xdB])
  ...
  const renderCompareTool = (a: string, setA: (v: string) => void, b: string, setB: (v: string) => void, equal: boolean, lenA: number, lenB: number, title: string, desc: string, language: 'plaintext' | 'json' | 'xml', fmtA: string, fmtB: string) => {
    return <AppTool theme={theme} setTheme={setTheme} tool={tool} onSelectTool={setTool}><ToolHead title={title} desc={desc} /><div className="panels"><section className="panel"><div className="panel-head"><div className="panel-title"><FileText size={16} /><h2>String A</h2></div><button className="quiet-button" onClick={() => setA('')} aria-label="Clear A"><RotateCcw size={14} /></button></div><div className="plain-editor-wrap"><textarea value={a} onChange={event => setA(event.target.value)} className="plain-editor" spellCheck={false} aria-label="String A" /></div><div className="panel-foot"><span className="meta-text">{a.length} characters</span></div></section><section className="panel"><div className="panel-head"><div className="panel-title"><FileText size={16} /><h2>String B</h2></div><button className="quiet-button" onClick={() => setB('')} aria-label="Clear B"><RotateCcw size={14} /></button></div><div className="plain-editor-wrap"><textarea value={b} onChange={event => setB(event.target.value)} className="plain-editor" spellCheck={false} aria-label="String B" /></div><div className="panel-foot"><span className="meta-text">{b.length} characters</span></div></section><OutPanel icon={<Check size={16} />} title="Result" className={equal ? '' : 'diff-panel'} style={{ gridColumn: '1 / -1' }} foot={<><span className={`status-dot ${equal ? '' : 'error'}`} /><span className="meta-text">{equal ? 'Equal' : 'Different'}</span></>} actions={equal ? undefined : <><button className="quiet-button" onClick={() => diffNav.current?.prev()} aria-label="Previous change" title="Previous change"><ChevronUp size={15} /></button><button className="quiet-button" onClick={() => diffNav.current?.next()} aria-label="Next change" title="Next change"><ChevronDown size={15} /></button></>}>{equal ? <div className="diagnostic ok" style={{ marginBottom: 0 }}><div className="diagnostic-icon"><Check size={16} /></div><div><strong>Strings are identical</strong><p>{lenA} characters on line 1 &middot; no differences found</p></div></div> : <div className="monaco-diff-wrap"><MonacoDiff original={fmtA} modified={fmtB} language={language} theme={theme} navRef={diffNav} /></div>}</OutPanel></div><ToolBar action="Swap A ↔ B" icon={<ArrowLeftRight size={14} />} onAction={() => { const t = a; setA(b); setB(t) }} clear="Clear both" onClear={() => { setA(''); setB('') }} output={a} /></AppTool>
  }
  ...
  if (tool === 'compare') return renderCompareTool(cmpA, setCmpA, cmpB, setCmpB, cmp.equal, cmp.lenA, cmp.lenB, TOOLS.compare.title, TOOLS.compare.desc, 'plaintext', cmpA, cmpB)
  if (tool === 'jsondiff') return renderCompareTool(jdA, setJdA, jdB, setJdB, jdCmp.equal, jdCmp.lenA, jdCmp.lenB, TOOLS.jsondiff.title, TOOLS.jsondiff.desc, 'json', jdFmt[0], jdFmt[1])
  if (tool === 'xmldiff') return renderCompareTool(xdA, setXdA, xdB, setXdB, xdCmp.equal, xdCmp.lenA, xdCmp.lenB, TOOLS.xmldiff.title, TOOLS.xmldiff.desc, 'xml', xdFmt[0], xdFmt[1])
