...
export default function Home() {
  ...
  if (tool === 'yaml') return <AppTool theme={theme} setTheme={setTheme} tool={tool} onSelectTool={setTool}><ToolHead title={TOOLS.yaml.title} desc={TOOLS.yaml.desc}><div className="format-switch"><button className={yamlMode === 'to' ? 'selected' : ''} onClick={() => setYamlMode('to')}>JSON → YAML</button><button className={yamlMode === 'from' ? 'selected' : ''} onClick={() => setYamlMode('from')}>YAML → JSON</button></div></ToolHead><div className="panels"><IoPanel icon={<Braces size={16} />} title="Input" label={yamlMode === 'to' ? 'JSON' : 'YAML'} value={yamlIn} onChange={setYamlIn} onClear={() => setYamlIn('')} placeholder={yamlMode === 'to' ? 'Paste JSON' : 'Paste YAML'} /><OutPanel icon={<Code2 size={16} />} title="Output" label={yamlMode === 'to' ? 'YAML' : 'JSON'}>{yamlOut.error ? <div className="diagnostic"><div className="diagnostic-icon"><AlertTriangle size={16} /></div><div><strong>Unable to convert</strong><p>{yamlOut.error}</p></div></div> : <pre className="raw-view">{yamlOut.value}</pre>}</OutPanel></div><ToolBar output={yamlOut.value} /></AppTool>
  if (tool === 'jsonxml') return <AppTool theme={theme} setTheme={setTheme} tool={tool} onSelectTool={setTool}><ToolHead title={TOOLS.jsonxml.title} desc={TOOLS.jsonxml.desc}><div className="format-switch"><button className={jxmlMode === 'json' ? 'selected' : ''} onClick={() => setJxmlMode('json')}>JSON → XML</button><button className={jxmlMode === 'xml' ? 'selected' : ''} onClick={() => setJxmlMode('xml')}>XML → JSON</button></div></ToolHead><div className="panels"><IoPanel icon={<Braces size={16} />} title="Input" label={jxmlMode === 'json' ? 'JSON' : 'XML'} value={jxmlIn} onChange={setJxmlIn} onClear={() => setJxmlIn('')} placeholder={jxmlMode === 'json' ? 'Paste JSON' : 'Paste XML'} /><OutPanel icon={<Code2 size={16} />} title="Output" label={jxmlMode === 'json' ? 'XML' : 'JSON'}>{jxmlOut.error ? <div className="diagnostic"><div className="diagnostic-icon"><AlertTriangle size={16} /></div><div><strong>Unable to convert</strong><p>{jxmlOut.error}</p></div></div> : <pre className="raw-view">{jxmlOut.value}</pre>}</OutPanel></div><ToolBar output={jxmlOut.value} /></AppTool>
  if (tool === 'csv') return <AppTool theme={theme} setTheme={setTheme} tool={tool} onSelectTool={setTool}><ToolHead title={TOOLS.csv.title} desc={TOOLS.csv.desc}><div className="format-switch"><button className={csvMode === 'json' ? 'selected' : ''} onClick={() => setCsvMode('json')}>JSON → CSV</button><button className={csvMode === 'csv' ? 'selected' : ''} onClick={() => setCsvMode('csv')}>CSV → JSON</button></div></ToolHead><div className="panels"><IoPanel icon={<Table size={16} />} title="Input" label={csvMode === 'json' ? 'JSON' : 'CSV'} value={csvIn} onChange={setCsvIn} onClear={() => setCsvIn('')} placeholder={csvMode === 'json' ? 'Paste JSON array' : 'Paste CSV'} /><OutPanel icon={<Table size={16} />} title="Output" label={csvMode === 'json' ? 'CSV' : 'JSON'}>{csvOut.error ? <div className="diagnostic"><div className="diagnostic-icon"><AlertTriangle size={16} /></div><div><strong>Unable to convert</strong><p>{csvOut.error}</p></div></div> : <pre className="raw-view">{csvOut.value}</pre>}</OutPanel></div><ToolBar output={csvOut.value} /></AppTool>
  ...
  if (tool === 'url') return <AppTool theme={theme} setTheme={setTheme} tool={tool} onSelectTool={setTool}><ToolHead title={TOOLS.url.title} desc={TOOLS.url.desc}><div className="format-switch"><button className={urlMode === 'parse' ? 'selected' : ''} onClick={() => setUrlMode('parse')}>Parse</button><button className={urlMode === 'encode' ? 'selected' : ''} onClick={() => setUrlMode('encode')}>Encode</button><button className={urlMode === 'decode' ? 'selected' : ''} onClick={() => setUrlMode('decode')}>Decode</button></div></ToolHead><div className="panels"><IoPanel icon={<Link size={16} />} title="URL" value={urlIn} onChange={setUrlIn} onClear={() => setUrlIn('')} placeholder="https://api.example.com/v2/orders?x=1" /><OutPanel icon={<Link size={16} />} title={urlMode === 'parse' ? 'Components' : urlMode === 'encode' ? 'Encoded' : 'Decoded'}>{urlMode === 'parse' ? (urlRes.error ? <div className="diagnostic"><div className="diagnostic-icon"><AlertTriangle size={16} /></div><div><strong>Invalid URL</strong><p>Enter a URL with a scheme (https://...)</p></div></div> : urlRes.parts.map(p => <div key={p.label} className="match-row"><span className="match-index">{p.label}</span><span className="match-value">{p.value}</span></div>)) : <pre className="raw-view">{urlTransformed}</pre>}</OutPanel></div><ToolBar output={urlMode === 'parse' ? JSON.stringify(urlRes.parts) : urlTransformed} /></AppTool>
