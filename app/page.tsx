...
export default function Home() {
  ...
  <section className="content"><div className="content-head"><div><div className="breadcrumbs">TOOLS <span>/</span> FORMATTER</div><h1>JSON & XML Formatter</h1></div><div className="format-switch"><button className={format === 'JSON' ? 'selected' : ''} onClick={() => { setFormat('JSON'); setInput(sampleJson) }}><FileJson size={15}/> JSON</button><button className={format === 'XML' ? 'selected' : ''} onClick={() => { setFormat('XML'); setInput(sampleXml) }}><FileCode2 size={15}/> XML</button></div></div>
  <div className="panels"><section className="panel editor-panel"><div className="panel-head"><div className="panel-title"><Code2 size={16}/><h2>Editor</h2><span className="format-label">{format}</span></div><div className="panel-actions"><button className="quiet-button" onClick={() => setInput('')} aria-label="Clear editor"><RotateCcw size={14}/></button></div></div><div className="editor-wrap"><div className="line-numbers">{lines.map((_, i) => <span key={i} className={diagnostic?.line === i + 1 ? 'number-error' : ''}>{String(i + 1).padStart(2, '0')}</span>)}</div><div className="code-editor"><pre aria-hidden="true">{editorLines}</pre><textarea spellCheck={false} value={input} onChange={e => setInput(e.target.value)} onScroll={e => { const t = e.currentTarget; const m = t.previousElementSibling as HTMLElement | null; if (m) { m.scrollTop = t.scrollTop; m.scrollLeft = t.scrollLeft } }} className="editor" wrap="soft" aria-label={`${format} editor`}/></div></div><div className="panel-foot"><span className="meta-text">{input.length} characters</span>{diagnostic && <span className="error-inline">Line {diagnostic.line}, col {diagnostic.column}</span>}<div className="panel-buttons"><button className="outline-button" onClick={minify}><Minimize2 size={14}/> Minify</button><button className="primary-button" onClick={pretty}><Wand2 size={14}/> Beautify</button></div></div></section>
